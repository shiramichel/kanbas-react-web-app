      ...
      if (quizId) {
        try {
          ...
          const scoreResponse = await axios.get(
            `${process.env.REACT_APP_REMOTE_SERVER}/api/scores/${userLoginId}/quiz/${quizId}`
          );
  ...
  const handleSubmitQuiz = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_REMOTE_SERVER}/api/scores`,
        submissionData
      );
      if (response.data) {
