...
export default function QuizListScreen({ userRole }) {
  ...
  const handlePublishQuiz = async (quizId, published) => {
    try {
      const updatedQuiz = await updateQuiz(cid, quizId, {
        published: !published,
      });
  ...
  return (
    <div id="wd-quizzes">
      ...
      {quizzes.length === 0 ? (
        userRole === "FACULTY" && (
          <p className="no-quizzes-message">
            No quizzes available. Click the "+ Quiz" button to add a new quiz.
          </p>
        )
      ) : (
          ...
          {quizzes.map(
            (quiz) =>
              (userRole === "FACULTY" || quiz.published) && ( // Check if user is Faculty or quiz is published
                <li
                  key={quiz._id}
                  className="wd-quiz-item list-group-item p-3 ps-1 d-flex align-items-center green-border-left"
                >
                  <div
                      ...
                      <div className="quiz-status">
                        {quiz.published ? (
                          <FaCheckCircle
                            ...
                            onClick={() =>
                              handlePublishQuiz(quiz._id, quiz.published)
                            }
                          />
                        ) : (
                          <FaTimes
