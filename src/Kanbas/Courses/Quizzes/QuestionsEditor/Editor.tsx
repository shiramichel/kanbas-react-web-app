...
export default function Editor(
  { question, handleCancel, pointval, setPointval }:
    {
      question: any;
      handleCancel: () => void;
      pointval: number;
  ...
  const saveQuestion = async (question: any) => {
    if (question.options.length === 0) {
      setErrorMessage("Please enter at least one answer.");
      return;
    } else if (question.options.filter((a: any) => a.correct).length === 0) {
      setErrorMessage("Please select a correct answer.");
      return;
    } else if (question.points < 0 || !question.points) {
    setErrorMessage("Please enter a valid point value.");
    return;
  }
...
function updatePointval(question: any): any {
