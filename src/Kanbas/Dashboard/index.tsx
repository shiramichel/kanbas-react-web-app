...
export default function Dashboard({ courses, course, setCourse, addNewCourse,
  ...
  const addUserCourse = async () => {
    // check course number is unique (required unique in schema)
    const allCourses = await fetchAllCourses();
    const isUnique = allCourses.filter((c: any) => c.number === course.number).length === 0;
    if (!isUnique) {
      setErrorMessage(`Course number ${course.number} already exists. Please enter a different course number.`);
      return;
    }
  ...
  return (
    <div id="wd-dashboard">
      ...
      {currentUser.role === 'FACULTY' ?
        ...
        : <Link to="/Kanbas/Enroll"><button className="btn btn-primary m-1 mb-3">Find Courses</button></Link>
      }<br />
